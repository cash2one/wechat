...
{  
	...
	private function checkSignature($str='')
	{
		...
		if( $tmpStr == $signature ){
			$this->log( "checkSignature success : " . $tmpStr . "-------" . $signature . "------");
			return true;
		}else{
			$this->log( "checkSignature failed : " . $tmpStr . "-------" . $signature . "------");
			return false;
		}
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
			...
			if (!$json || isset($json['errcode'])) {
				...
				$this->log("access_token errorcode:" . $this->errCode . " " . $this->errMsg);
				return false;
			}
	...
	public function getServerIp(){
		...
		{
			$this->log("access_token is not get");
			return false;
		}
		...
		if ($result)
		{
			...
			if (!$json || isset($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				$this->log("access_token errorcode:" . $this->ErrorCode . " " . $this->errMsg);
				return false;
			}
			return $json['ip_list'];
		}
	...
	public function createMenu($data){
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				$this->log("create menu error:" . "errCode:" . $this->errCode . "errMsg:" . $this->errMsg);
				return false;
			}
			return true;
		}
	...
	public function uploadMedia($data, $type){
		if (!$this->access_token && !$this->checkAuth())
		{
			$this->log("uploadMedia, checkAuth failed");
			return false;
		} 
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				$this->log("uploadMedia failed," . "errCode=" . $this->errCode . ",errMsg=" . $this->errMsg);
				return false;
			}
			return $json;
		}
		$this->log("uploadMedia, failed");
		return false;
	}
